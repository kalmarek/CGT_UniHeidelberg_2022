...
# ╔═╡ 9cb420c6-edd0-4528-a7df-0b5da5169be2
...
# ╔═╡ 838d2dae-aa1b-4705-95b6-f5ea078c149f
begin
	struct CyclePermutation <: AbstractPermutation
		cycles::Vector{Vector{Int}}
	
		function CyclePermutation(c::Vector{Vector{Int64}}, check=true)
			if check
				m = mapreduce(maximum, max, c, init=0)
		...
		function CyclePermutation(images::Vector{Int64}, check=true)
			if check
				f = falses(length(images))
			...
			new(cycle_decomposition(Permutation(images)))
		end
	end

	function (s::CyclePermutation)(i::Integer)
		img = i
		for v in reverse(s.cycles)
			for n in 1:length(v)
				img != v[n] && continue
	...
	function degree(s::CyclePermutation)
		return mapreduce(c->length(c) == 1 ? 1 : maximum(c), max, s.cycles, init=1)
	end
end
...
# ╔═╡ 13b6b7bd-3bf7-4a36-9b85-a097c52709c2
axes(-5:4)

# ╔═╡ c161a5b3-7863-4abc-852a-443731d988db
let b = CyclePermutation([[1], [2], [3]])
	degree(b)
end

# ╔═╡ fc5d68c2-6575-4619-857c-909fbd7e10a6


# ╔═╡ 62f7fc72-990b-43d3-bce6-e61b31e3b511
begin

	function Base.:(^)(s::CyclePermutation, i::Integer)
		t = one(s)
...
# ╔═╡ faa71d4f-3de9-4723-972f-34d848d22e74
s = CyclePermutation([[1, 2, 3], [5, 6]])

# ╔═╡ 614ae800-3bf4-4c2a-af41-c9208e47356d
t = CyclePermutation([3, 1, 2, 4, 6, 5])

# ╔═╡ ba90101c-9ac6-498a-89af-1b45cddfacaf
s(5)

# ╔═╡ a0303c83-889a-45f7-8fcb-d9260fb70590
begin
	@assert s*inv(s) == one(s)
	@assert t == inv(s)
	@assert s^6 == one(s)
	@assert s*CyclePermutation([[4, 5]]) == CyclePermutation([[1, 2, 3], [4, 5, 6]])
end

# ╔═╡ c7865f8a-55cb-4900-b3ec-644e4ec56260
r = Permutation([3, 1, 2])

# ╔═╡ ca098a33-d475-457a-9401-ef86cf94b254
begin
	"""
	The multiplying function takes one parameter of type P<:AbstractPermutation and another plainly of type AbstractPermutation. Thus two different subtypes of AbstractPermutation can be inputted, and since they are only adressed through functions defined by the interface, they are compatible.
	I actually changed the type of the second parameter to also be P at first (because I thought it might be more correct without thinking too much about it), and so this multiplication didn't work since the function expected both parameters to be of the same type.
	"""
...
# ╔═╡ 9d3d10d2-9db4-4bd2-b340-10f3c3fc9a67
...
# ╔═╡ 859b93d2-1fc6-4c6f-96c9-c10cc0b1cfe0
methods(transversal)
...
function representative(y, S::AbstractVector{<:GroupElement}, Δ, Sch, action=^)
	...
	while (gen_index = Sch[current]) != 0
		current = action(current, inv(S[gen_index]))
		g = S[gen_index] * g
	end
...
# ╔═╡ 837ec779-15a1-42e3-b58f-7791009244b6
begin
	...
	abstract type AbstractTransversal <: AbstractOrbit end
end

# ╔═╡ 90d099bf-45bd-492b-b359-7ebc354e6379
begin
	struct Transversal <: AbstractTransversal
		
	end
	"""
	-query for group element
	"""
end

# ╔═╡ f865dee4-7ea8-4f2d-8541-8d4a6ccf63f7
# IDEs:
# vscode
# atom
...
# ╔═╡ b33f7a22-4678-4fd8-af48-6280abe6acca
function representative2(y, S::AbstractVector{<:GroupElement}, Δ, Sch, action=^)
	@assert !isempty(S)
	current = y
	g = one(first(S))

	while (s = Sch[current]) != one(first(S))
		current = action(current, inv(s))
		g = s * g
	end
...
# ╔═╡ 7eb852ba-d947-495d-b0a9-9f225b1392eb
function Base.:(^)(n::Integer, g::AbstractPermutation)
	return g(n)
end

# ╔═╡ 13abc38b-a0a2-46ac-a58a-24fead1f6702
begin
# this content is copied from `src/AbstractPermutations.jl`
	"""
    AbstractPermutation
