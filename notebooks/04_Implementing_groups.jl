...
function stabilizer_chain(G::PermutationGroup{P,T}) where {P,T}
	if !isdefined(G, :stab_chain)
		G.stab_chain = schreier_sims(T, gens(G))
	end
	return G.stab_chain
end
```
...
```julia
	if !isdefined(G, :stab_chain)
    	G.stab_chain = schreier_sims(T, gens(G), order(G))
	end
```
...
```julia
    if !isdefined(G, :stab_chain)
        G.stab_chain = if !isdefined(G, :order)
                schreier_sims(T, gens(G))
            else
                schreier_sims(T, gens(G), order(G))
            end
        end
    end
```
...
begin
	mutable struct PermutationGroup{P, T<:CGT.AbstractTransversal} <: AbstractPermGroup{P}
		...
		# Constructor where:
		# only gens are known
		PermutationGroup{T}(gens::AbstractVector{P}) where {P,T} = 
			new{P,T}(gens)
		# gens and order are known
		PermutationGroup{T}(gens::AbstractVector{P}, order::Integer) where {P,T} =
			new{P,T}(gens, order)
...
To modify a `struct` asynchronously one can use locks to avoid data races/object inconsistency (at the deadlocks). For example if you run this code
...
the time block will execute in `8/Threads.nthreads()` seconds, but You will occasionally see a number different than `n=8` . This is because a thread read/stored value in `s`  while `s` was modified/held by a different thread. This can be alleviated by e.g. using `s = Threads.Atomic{Int}(0)` but this solution only applies to `primitive structs`. One general solution is to use locks to limit the number of simultaneous assignments to `s`:
...
md"
# Exercices:
> **Exercise 1**: Implement a method to reconstruct group element from the images of a given basis of a `StabilizerChain`.
> Use this to implement `rand` method for obtaining uniformly random elements from the group generated by a `StabilizerChain`.

> **Exercise 2**: Implement:
>
> 1. simple backtrack search and use it to finalize the iteration protocol for `PermutationGroups`;
>
> 2. a general backtrack with _an oracle_ to descend to/prune branches. What kind of information should we pass to it (except current node represented by an element `g`)? (note: there is no absolutely general search, the oracle is very often problem dependent).

> **Exercise 3**:
>
> 1. Implement a method that takes `G::AbstractPermGroup` and `g ∈ G` and  reconstructs a word in the original generators of `G` which is equal to `g`.
>
> 2. Use the above method to solve the standard puzzle games in `test/puzzles.jl`
