...
```
Here we make promise that images of permutations will be stored as vectors of `Int`s and `length(images)` determines group `Sym(n)` a permutation belongs to. 
...
begin
	...
	struct Permutation
		...
		function Permutation(v::Vector{Int}, check=true)
			if check
...
# ╔═╡ 48bf42cd-2f4e-49f9-803b-3556ad780182
σ

# ╔═╡ c546542c-2d69-45ed-9c7b-6cdf48fa6a43
τ = Permutation([1,3,2,4])
